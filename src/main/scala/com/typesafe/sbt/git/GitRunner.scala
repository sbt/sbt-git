...
trait GitInstance {
   ...
   */
  def apply(args: String*)(repo: File, log: Logger): String
  ...
  def currentTopTagOrNone(cwd: File, log: Logger): Option[String] = {
    val sha = headCommit(cwd, log)
    // TODO - This seems prone to failure and risky.  JGit is a better option.
    try Some(apply("describe", "--tags", "--exact-match", sha)(cwd, log))
    catch {
